// During local dev we proxy via Vite to bypass browser CORS
const isLocal = typeof window !== 'undefined' && window.location && window.location.hostname === 'localhost';
const BASE_PRIVATE = isLocal ? "/__proxy/judgeme-private/api/v1" : "https://api.judge.me/api/v1";
const BASE_PUBLIC = isLocal ? "/__proxy/judgeme-public/api/v1" : "https://judge.me/api/v1";

const SHOP_DOMAIN = "roseatefoods.myshopify.com";
const PUBLIC_TOKEN = "K7ZY4WrP9vG9CCuFUROB_kmzq0g";
// If you have a private API token, set it via env (Vite) and fall back to PUBLIC_TOKEN
const PRIVATE_TOKEN = (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_JUDGEME_PRIVATE_TOKEN) || PUBLIC_TOKEN;
...
export async function getProductByExternalId(externalId) {
    const url = `${BASE_PRIVATE}/products/-1?shop_domain=${encodeURIComponent(SHOP_DOMAIN)}&api_token=${encodeURIComponent(PRIVATE_TOKEN)}&external_id=${encodeURIComponent(String(externalId))}`;
    return fetchJson(url);
}

// PRIVATE: list reviews by internal product_id (pagination)
export async function listReviews({ productId, page = 1, perPage = 20 }) {
    const url = `${BASE_PRIVATE}/reviews?shop_domain=${encodeURIComponent(SHOP_DOMAIN)}&api_token=${encodeURIComponent(PRIVATE_TOKEN)}&product_id=${productId}&page=${page}&per_page=${perPage}`;
    return fetchJson(url);
}
...
export async function sendManualReviewRequest({ reviewer_name, reviewer_email, shopify_product_id, product_handle, fulfilled_at }) {
    ...
    const bodyJson = JSON.stringify({ api_token: PRIVATE_TOKEN, reviewer_name, reviewer_email, shopify_product_id, product_handle, fulfilled_at });
    return fetchJson(url, { method: "POST", body: bodyJson });
}
...
// PUBLIC: product review widget by external_id (Shopify product id)
export async function getProductReviewWidgetByExternalId(externalId) {
    const url = `${BASE_PUBLIC}/widgets/product_review?shop_domain=${encodeURIComponent(SHOP_DOMAIN)}&api_token=${encodeURIComponent(PUBLIC_TOKEN)}&external_id=${encodeURIComponent(String(externalId))}`;
    return fetchJson(url);
}
